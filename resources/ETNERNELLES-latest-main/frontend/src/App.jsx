...
const DICTIONARY = {
  ...
  es: {
    ...
    hero: {
      aria: 'Carrusel de moda destacado', iframeTitle: 'Carrusel de pasarela EOEX', kicker: 'La pasarela te espera', heading: 'EOEX', subheading: 'G\u00e9nesis de Pasarela', copy: 'Representaci\u00f3n \u00e9tica para la nueva generaci\u00f3n de talento global de moda.', cta: 'Descubrir la Pasarela',
    },
    about: {
  ...
  it: {
    ...
    hero: { aria: 'Carousel moda in evidenza', iframeTitle: 'Carousel passerella EOEX', kicker: 'La passerella ti aspetta', heading: 'EOEX', subheading: 'Genesi della Passerella', copy: 'Rappresentanza etica per la nuova generazione di talenti globali della moda.', cta: 'Scopri la Passerella' },
    about: {
  ...
  pt: {
    ...
    hero: { aria: 'Carrossel de moda em destaque', iframeTitle: 'Carrossel de passarela EOEX', kicker: 'A passarela espera por ti', heading: 'EOEX', subheading: 'G\u00e9nese de Passarela', copy: 'Representa\u00e7\u00e3o \u00e9tica para a nova gera\u00e7\u00e3o de talento global de moda.', cta: 'Descobrir a Passarela' },
    about: {
...
function App() {
  ...
  const [visibleArchiveImagesCount, setVisibleArchiveImagesCount] = useState(18);
  ...
  const archiveImages = useMemo(() => {
    ...
    return activeArchiveSection.items.filter((item) => {
      ...
      const searchable = [item.name, item.timeline, item.context, item.imageName]
        .join(' ')
        .toLowerCase();
  ...
  useEffect(() => {
    ...
    setArchiveSearch('');
    setVisibleArchiveImagesCount(18);
  }, [archiveTabId]);
      ...
      <main>
        ...
        <section id="archive" className="section archive-section">
          ...
          <div className="archive-media reveal">
            ...
            <div className="archive-tabs" role="tablist" aria-label="Gallery sections">
            ...
            <div className="archive-filters">
              <div className="archive-timeline-filters" role="tablist" aria-label="Timeline filters">
              ...
              <input
                ...
                placeholder="Filter by name or context"
                aria-label="Filter gallery cards"
              />
            </div>
              ...
              {visibleArchiveImages.map((image) => (
                <button key={image.id} type="button" className="archive-image-card" onClick={() => setActiveArchiveImage(image)}>
                  <img
                    ...
                    alt={`${image.name} ${image.timeline}`}
                    onError={(event) => handleImageFallback(event, 'video')}
                  />
                  <span>{image.name}</span>
                  <small>{image.timeline}</small>
                </button>
              ))}
            </div>
      ...
      {activeArchiveImage && (
        <div className="modal-backdrop" role="presentation" onClick={() => setActiveArchiveImage(null)}>
          <article className="modal-card archive-image-modal" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
            ...
            {activeArchiveImage.context && <p><strong>Context:</strong> {activeArchiveImage.context}</p>}
            {activeArchiveImage.description && <p>{activeArchiveImage.description}</p>}
            {activeArchiveImage.photographer && <p><strong>Photographer:</strong> {activeArchiveImage.photographer}</p>}
          </article>
        </div>
      )}
    </div>
